class Api::Events::RegistrationsController < Api::Events::ApplicationController
  ...
  def create
    ...
    if params[:event_registration][:user_id] == current_user.id.to_s
      ...
      if @event_form.save
        render json: { role: @event_form.model.role, participant: @event_form.model.user.to_json(only: [:id, :ticket, :first_name, :last_name]), avatar_url: @event_form.model.user.decorate.element_avatar.url }
      else
        head :bad_request
      end
    else
